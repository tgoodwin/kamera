...
func headlessDomain(z *ZookeeperCluster) string {
	return fmt.Sprintf("%s.%s.svc.%s", headlessSvcName(z), z.GetNamespace(), z.GetKubernetesClusterDomain())
}

func headlessSvcName(z *ZookeeperCluster) string {
...
func MakeStatefulSet(z *ZookeeperCluster) *appsv1.StatefulSet {
...
func makeZkPodSpec(z *ZookeeperCluster, volumes []v1.Volume) v1.PodSpec {
...
func MakeClientService(z *ZookeeperCluster) *v1.Service {
...
func MakeAdminServerService(z *ZookeeperCluster) *v1.Service {
	ports := z.ZookeeperPorts()
	svcPorts := []v1.ServicePort{
...
func MakeConfigMap(z *ZookeeperCluster) *v1.ConfigMap {
	return &v1.ConfigMap{
		TypeMeta: metav1.TypeMeta{
			Kind:       "ConfigMap",
			APIVersion: "v1",
...
func MakeHeadlessService(z *ZookeeperCluster) *v1.Service {
	ports := z.ZookeeperPorts()
	svcPorts := []v1.ServicePort{
		{Name: "tcp-client", Port: ports.Client},
...
func makeZkConfigString(z *ZookeeperCluster) string {
	ports := z.ZookeeperPorts()
	var zkConfig = ""
...
func makeZkEnvConfigString(z *ZookeeperCluster) string {
	ports := z.ZookeeperPorts()
...
func makeService(name string, ports []v1.ServicePort, clusterIP bool, external bool, annotations map[string]string, z *ZookeeperCluster) *v1.Service {
	var dnsName string
...
func MakePodDisruptionBudget(z *ZookeeperCluster) *policyv1.PodDisruptionBudget {
	pdbCount := intstr.FromInt32(z.Spec.MaxUnavailableReplicas)
...
func MakeServiceAccount(z *ZookeeperCluster) *v1.ServiceAccount {
