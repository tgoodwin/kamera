# Kratix Static Dependency Graph Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Produce `examples/kratix/kratix-dependency-graph.json` describing controller/resource dependencies for the Kratix repo using the static graph spec.

**Architecture:** Manually inspect Kratix controller constructors and reconcile loops to identify primary resources, watches, ownership, reads, and writes. Emit a single JSON graph using canonical GVK strings and the raw node/edge format defined in `docs/plans/2026-01-27-static-dependency-graph-design.md`.
...
**Files:**
- Modify: `examples/kratix/kratix-dependency-graph.json`
...
Run:
```bash
rg -n "NewController|NewReconciler|SetupWithManager|Add.*Controller|WithController" ~/projects/kratix/cmd ~/projects/kratix/internal ~/projects/kratix/pkg
```
...
**Files:**
- Create/Modify: `examples/kratix/kratix-dependency-graph.json`
...
Run:
```bash
jq . examples/kratix/kratix-dependency-graph.json >/dev/null
```
