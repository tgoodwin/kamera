...
Success prints `PASS` for the two converging cases and `OBSERVED` for the
bounded KRO-2 result, then writes traces, logs, oracle JSON, and
`section61.tsv`.
...
- **Section 6.1:** all three configurations rerun and check their observable
  outcomes. KRO-2 reports its outcome and bounded status separately because
