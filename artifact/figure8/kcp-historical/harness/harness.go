...
func newKCPHarness(scheme *runtime.Scheme, recorder replay.EffectRecorder, state []runtime.Object) (*kcpHarness, func(), error) {
	...
	// NewAPIBinder takes 9 params: client, 3 logicalClusterInformers, 2 workspaceType, 1 apiBinding, 2 apiExport.
	// In single-shard harness, local/global informers are the same.
	apiBinderController, err := initialization.NewAPIBinder(
		h.kcpClient,
		logicalClusterInformer, logicalClusterInformer, logicalClusterInformer, // local, list, global
	...
	defaultAPIBindingCtrl, err := defaultapibinding.NewDefaultAPIBindingController(
		h.kcpClient,
		logicalClusterInformer, logicalClusterInformer, // local + global
		workspaceTypeInformer, workspaceTypeInformer,   // local + global
		apiBindingInformer,
		apiExportInformer, apiExportInformer, // local + global
	)
