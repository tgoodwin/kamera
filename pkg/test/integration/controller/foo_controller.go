...
func (r *FooReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	switch foo.Spec.Mode {
	case "A":
		if foo.Status.State == "" || foo.Status.State == "B-1" {
			foo.Status.State = "A-1"
			updated = true
		} else if foo.Status.State == "A-1" {
			foo.Status.State = "A-2"
			updated = true
		} else if foo.Status.State == "A-2" {
			foo.Status.State = "A-Final"
			updated = true
		}
	case "B":
		if foo.Status.State == "" || foo.Status.State == "A-1" {
			foo.Status.State = "B-1"
			updated = true
		} else if foo.Status.State == "B-1" {
			foo.Status.State = "B-2"
			updated = true
		} else if foo.Status.State == "B-2" {
