...
func TestResourceConflictValidator_Create(t *testing.T) {
	...
	err := validator.ValidateOperation(event.CREATE, obj1, nil)
	...
	err = validator.ValidateOperation(event.CREATE, obj2, nil)
	...
	err = validator.ValidateOperation(event.CREATE, obj3, nil)
	...
	err = validator.ValidateOperation(event.CREATE, obj4, nil)
...
func TestResourceConflictValidator_Get(t *testing.T) {
	...
	_ = validator.ValidateOperation(event.CREATE, obj, nil)

	// Test case 1: Get existing object (should succeed)
	err := validator.ValidateOperation(event.GET, obj, nil)
	...
	err = validator.ValidateOperation(event.GET, objNotFound, nil)
...
func TestResourceConflictValidator_List(t *testing.T) {
	...
	_ = validator.ValidateOperation(event.CREATE, obj1, nil)
	_ = validator.ValidateOperation(event.CREATE, obj2, nil)

	// Test case: List operation always succeeds
	dummyObj := createTestObject("dummy", "default", gvk)
	err := validator.ValidateOperation(event.LIST, dummyObj, nil)
...
func TestResourceConflictValidator_Update(t *testing.T) {
	...
	err := validator.ValidateOperation(event.UPDATE, obj, nil)
	...
	objNotFound := createTestObject("nonexistent", "default", gvk)
	err = validator.ValidateOperation(event.UPDATE, objNotFound, nil)
...
func TestResourceConflictValidator_Delete(t *testing.T) {
	...
	err := validator.ValidateOperation(event.MARK_FOR_DELETION, obj, nil)
...
func TestResourceConflictValidator_Patch(t *testing.T) {
	...
	err := validator.ValidateOperation(event.PATCH, obj, nil)
	...
	objNotFound := createTestObject("nonexistent", "default", gvk)
	err = validator.ValidateOperation(event.PATCH, objNotFound, nil)
...
func TestResourceConflictValidator_Apply(t *testing.T) {
	...
	obj := createTestObject("test-deployment", "default", gvk)
	err := validator.ValidateOperation(event.APPLY, obj, nil)
...
func TestResourceConflictValidator_ApplyStatusRequiresExistingObject(t *testing.T) {
	validator := newValidator()
	...
	err := validator.ValidateOperation(event.APPLY, obj, &EffectOptions{Subresource: "status"})
	assert.Error(t, err, "status apply should fail when object doesn't exist")
	assert.True(t, apierrors.IsNotFound(err), "Error should be NotFound")

	err = validator.ValidateOperation(event.GET, obj, nil)
	assert.True(t, apierrors.IsNotFound(err), "status apply should not create the object")
}
...
func TestResourceConflictValidator_DifferentObjectsWithSameName(t *testing.T) {
	...
	err := validator.ValidateOperation(event.CREATE, deployment, nil)
	...
	err = validator.ValidateOperation(event.CREATE, service, nil)
	assert.NoError(t, err, "Should be able to create service with same name as deployment")

	// Verify both exist
	err = validator.ValidateOperation(event.GET, deployment, nil)
	assert.NoError(t, err, "Deployment should exist")

	err = validator.ValidateOperation(event.GET, service, nil)
