...
3. **Create an ExplorerBuilder.** It lets you register reconciler implementations with the appropriate resource dependencies and lets you tune exploration parameters.
    ...
    - No staleness/perturbation injection is applied unless you explicitly set it in the explore config (for example: `cfg := eb.Config(); cfg.Perturbations["FooController"] = tracecheck.PerturbationConfig{...}; eb.SetConfig(cfg)`).
